...
#Change the GPIO that is able to be monitored for status
...
class Snoop(Thread):
    ...
    def run(self):
        ...
        if(GPIO.input(23) ==1):
            ...
            if self.verb > 0:
                logging.info("Plugin GPIO Read GPIO # ", GPIO % (GR,self.fname,G,GR,new_value,G))   
